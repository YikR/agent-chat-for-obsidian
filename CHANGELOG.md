...
- 自动生成的桌面 Bridge config 改为监听 `0.0.0.0`，让同一个 Bridge 进程可同时接受 Wi-Fi 局域网和 Tailscale/虚拟局域网访问。
- 设置页新增 `检测 Bridge 可达性`，可在桌面端或手机端检查当前 Bridge URL 是否可访问。
- 手机端仍不运行本地 CLI；配置完整时走 Bridge，配置不完整或不可达时保留工作台/派单兜底。
- 新增 Tailscale 地址识别、健康检查和双端设置入口测试。
