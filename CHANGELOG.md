...
- 新增 Bridge 候选地址列表，手机端检测和发送消息时会按顺序自动尝试局域网、Tailscale/虚拟局域网等多个地址。
- 成功连通的候选地址会自动写回为当前 `Bridge 地址`，下次优先使用最近可用路径。
- `一键初始化手机 Bridge` 会在检测到 Tailscale 和局域网地址时同时保存为候选地址。
- 桌面端检测仍固定检查本机 `127.0.0.1:<port>`，避免桌面端自测 MagicDNS 造成误判。
- 新增候选地址去重、fallback、健康检查、POST 探针和运行链路选中 URL 测试。
